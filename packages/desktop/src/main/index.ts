...
const APP_NAMES: Record<string, string> = {
  dev: "Семена - Агент Dev",
  beta: "Семена - Агент Beta",
  prod: "Семена - Агент",
}
...
const main = Effect.gen(function* () {
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "Семена - Агент Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("semena-agent://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("semena-agent")
  registerRendererProtocol()
  setDockIcon()
